/*==============================================================================
  ...
  BUG FIX 1 (G coding): csdid requires G=0 for never-treated units; the
    analysis file stores G=. for controls.  A temporary variable G_cs is
    created: G_cs = G for treated firms, G_cs = 0 for controls.

  BUG FIX 2 (colnames): the original code used
      local colnames : rownames r(table)
    which returns the statistic names (b se t p ll ul …), not the parameter
    names.  Fix: colnames r(table).

  BUG FIX 3 (local val syntax): the original code used
      local val = real(regexs(1)) if regexm(...)
    Stata's local command has no if qualifier; this silently ignores the
    condition and always evaluates regexs(1).  Fix: use flow-control if.

  Outputs per outcome:
    Output\ModernDID\excel\CSDID_<y>_-4_9.xlsx
...
program define csdid_es_one
    ...
    // BUG FIX 1: recode G=. -> G=0 for csdid's never-treated convention
    ...
    //                  cols = parameter names
    // Transpose so each row is one parameter.
    matrix M = r(table)'
    ...
    preserve
        ...
        // BUG FIX 2: use colnames r(table) to get parameter names.
        // (rownames r(table) would give b se t p ll ... which is wrong.)
        ...
        foreach nm of local colnames {
            local ++j
            // BUG FIX 3: proper flow-control if, not local ... if regexm(...)
            local val .
            if regexm("`nm'", "(-?[0-9]+)$") local val = real(regexs(1))
            cap replace tid = `val' in `j'
        }

        // Fallback to sequential numbering if label parsing yielded all missing
        ...
        // Confidence bounds: ll/ul come from svmat if r(table) has them
    ...
    // Pre-trend test (graceful if not supported by installed csdid version)
...
di as result "07_csdid.do complete."
