/*==============================================================================
  ...
  The TWFE is re-estimated here with explicit binary dummies (identical to
  01_twfe.do) so that honestdid can unambiguously identify the pre- and
  post-treatment event-study coefficients in e(b).

  Dummy layout (must match $HONESTDID_NPRE and $HONESTDID_NPOST):
    Pre : d_m4  d_m3  d_m2          →  t = -4, -3, -2   (numpre  = 3)
    Post: d_p0  d_p1  …  d_p9       →  t =  0 …  9      (numpost = 10)
  The 13 ES dummies are listed FIRST in the reghdfe call, so they occupy
  positions 1..13 in e(b) and e(V).

  Two sensitivity dimensions:
    Smoothness restriction (delta=sd): max second-difference of trend ≤ M
    Relative magnitudes   (delta=rm): post-violation ≤ Mbar × max-pre-violation
...
program define honestdid_one
    ...
    local mvec_sm    "$HONESTDID_MVEC_SMOOTH"
    ...
    // ── Explicit event-time dummies (same as 01_twfe.do) ───────────────────
    // Controls have all dummies = 0 and remain in the regression.
    foreach k in 4 3 2 {
        cap drop d_m`k'
    ...
    // ES dummies listed first → they occupy e(b) positions 1..(npre+npost)
    quietly reghdfe `y' `pre_dummies' `post_dummies' $CTRL_DEFAULT, ///
        absorb(id t) vce(cluster id)

    // ── Slice e(b) and e(V) to the ES dummies only ─────────────────────────
...
di as result "07_honest_did.do complete."
